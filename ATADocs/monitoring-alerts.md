...
|Alerta|Descrição|Resolução|Severidade|
|----|----|----|----|
|O ATA falhou ao enviar eventos para o SIEM especificado.|Isso significa que o centro do ATA não pode enviar atividades suspeitas e alertas de integridade para o SIEM.|Certifique-se de que suas [configurações do servidor Syslog estão definidas corretamente](setting-syslog-email-server-settings.md).|Baixa|
